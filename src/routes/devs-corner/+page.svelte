<script lang="ts">
	...
	// screenshots is now string[] per system — empty array shows placeholder
	const systems = [
		{
	...
	// Per-card carousel state — keyed by system id
			...
			<div class="tab-content">
				<div class="note-body">
					<p>
						...
						being angry about it and started asking a different question: <em>what am I actually handing down?</em>
					</p>
					<p>
...
<style>
	main { background: linear-gradient(170deg, rgb(13,13,13), rgb(20,20,20) 66%) }
	...
	.eyebrow { font-size: 0.55rem; letter-spacing: 5px; color: #7a5a20; text-transform: uppercase; margin: 0 0 0.6rem; }
	.page-title { font-size: 2rem; color: #e8b96a; letter-spacing: -0.3px; font-family: "Lexend"; text-transform: uppercase; margin: 0 0 1rem; }
	.rule { width: 100px; height: 1px; background: linear-gradient(90deg, transparent, #7a5a20, transparent); margin: 0 auto; }

	/* ── Tabs ── */
	.tabs { display: flex; gap: 6px; margin-bottom: 2rem; border-bottom: 1px solid rgba(92, 61, 30, 0.25); padding-bottom: 0; }
	.tab { font-family: 'Lexend'; font-size: 1rem; letter-spacing: -0.5px; text-transform: uppercase; color: #f2cc8f; background: none; border: none; border-bottom: 2px solid transparent; padding: 0.5rem 1rem 0.6rem; cursor: pointer; transition: color 0.12s, border-color 0.12s; margin-bottom: -1px; }
	.tab:hover { color: #486c52; }
	.tab.active { color: #558663; border-bottom-color: #558663; }
	.tab-content { min-height: 400px; }

	/* ── Section label ── */
	.section-label { font-size: 1.6rem; font-family: 'Lexend'; letter-spacing: -0.5px; text-transform: uppercase; color: #5c402a; margin: 0 auto 1rem; }

	/* ── Stat grid ── */
	.stat-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 8px; margin-bottom: 1.5rem; }
	.stat-card { background: rgba(26, 20, 12, 0.5); border: 1px solid #7e501e; border-radius: 6px; padding: 0.9rem 1rem; display: flex; flex-direction: column; gap: 4px; }
	.stat-value { font-size: 2rem; color: #cc951e; line-height: 1; font-family: 'Lexend'; }
	.stat-label { font-family: 'DePixel'; font-size: 1rem; letter-spacing: -0.5px; text-transform: uppercase; color: #f2cc8f; }

	/* ── Tools list ── */
	.tools-list { display: flex; flex-direction: column; gap: 0; border: 1px solid #7e501e; border-radius: 6px; overflow: hidden; margin-bottom: 0.5rem; background: rgba(26, 20, 12, 0.5); }
	.tool-row { display: grid; grid-template-columns: 1fr 2fr 3fr; gap: 12px; padding: 0.6rem 1rem; border-bottom: 1px solid rgba(92, 61, 30, 0.2); font-size: 1rem; align-items: baseline; }
	.tool-row:last-child { border-bottom: none; }
	.tool-name { color: #b18e41; }
	.tool-role { color: #987d51; font-style: italic; }
	.tool-note { color: #8e8c7f; font-size: 1rem; }

	/* ── Inspirations ── */
	.inspirations { border: 1px solid #7e501e; border-radius: 6px; padding: 1.2rem 1.4rem; margin-bottom: 0.5rem; background: rgba(26, 20, 12, 0.5); }
	.insp-row { display: grid; grid-template-columns: 1fr 1fr; border-bottom: 1px solid rgba(92, 61, 30, 0.2); padding: 0.5rem 0; font-size: 1rem; }
	...
	.tl-label { font-size: 0.75rem; letter-spacing: 3px; text-transform: uppercase; color: #c9973a; display: block; margin-bottom: 0.25rem; }
	.tl-desc { font-size: 1rem; color: #7a5a38; line-height: 1.55; font-style: italic; margin: 0; }

	/* ── Systems grid ── */
	.systems-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
	.sys-card { background: rgba(26, 20, 12, 0.5); border: 1px solid #3d2810; border-radius: 6px; overflow: hidden; display: flex; flex-direction: column; }
	.sys-header { padding: 0.8rem 1rem 0.6rem; border-bottom: 1px solid rgba(92, 61, 30, 0.3); display: flex; flex-direction: row-reverse; align-items: baseline; justify-content: space-between; height: fit-content; }
	.sys-tag { font-size: 0.75rem; text-transform: uppercase; color: #c49a4c; flex-shrink: 0; border: 1px solid #5d3e1a; padding: 4px 6px; border-radius: 3px; }
	.sys-title { font-size: 1.2rem; letter-spacing: 0em; font-family: 'Lexend'; text-transform: uppercase; color: #c9973a; font-weight: 400; }

	/* ── Carousel ── */
	.carousel { position: relative; width: 100%; aspect-ratio: 16/9; overflow: hidden; background: rgba(0,0,0,0.3); }
	.carousel-img { width: 100%; height: 100%; object-fit: contain; display: block; }
	.carousel-btn {
		position: absolute; top: 50%; transform: translateY(-50%);
		background: rgba(26, 20, 12, 0.75); border: 1px solid #5d3e1a;
		color: #c49a4c; font-size: 1.2rem; line-height: 1;
	...
	.carousel-dots { position: absolute; bottom: 6px; left: 50%; transform: translateX(-50%); display: flex; gap: 5px; }
	.dot { width: 5px; height: 5px; border-radius: 50%; background: rgba(196, 154, 76, 0.35); transition: background 0.15s; }
	.dot.active { background: #c49a4c; }

	.sys-screenshot-placeholder { width: 100%; aspect-ratio: 16/9; background: rgba(0,0,0,0.3); display: flex; align-items: center; justify-content: center; border-bottom: 1px solid rgba(92, 61, 30, 0.2); }
	.sys-screenshot-placeholder span { font-size: 0.5rem; letter-spacing: 3px; text-transform: uppercase; color: #3d2810; }

	.sys-body { padding: 0.8rem 1rem; flex: 1; }
	.sys-section-label { font-size: 1rem; letter-spacing: -0.5px; text-transform: uppercase; color: #7c5124; margin: 0 0 0.35rem; font-family: "Roboto Mono"; }
	.sys-section-label:not(:first-child) { margin-top: 0.75rem; }
	.sys-text { font-size: 1.2rem; color: #987d51; line-height: 1.4; margin: 0; font-family: 'Ubuntu Mono'; }
	.sys-note { font-size: 1rem; color: #7f7467; font-style: italic; margin: 0.6rem 0 0; line-height: 1.5; font-family: "Ubuntu Mono";}

	/* ── Philosophy tab ── */
	.note-body { color: #8e8c7f; font-size: 1.1rem; line-height: 1.5; margin-bottom: 0.5rem; font-family: "Roboto Mono"; letter-spacing: -0.3px; }
	.note-body p { margin-bottom: 1.3em; }
	.note-body em { color: #e8d4a8; font-style: italic; }
	.note-body strong { color: #e8b96a; font-size: 0.85em; }

	blockquote { border-left: 3px solid #7a5a20; padding: 0.8rem 0 0.8rem 1.5rem; margin: 1.5rem 0; color: #e8d4a8; font-style: italic; font-size: 0.95rem; line-height: 1.7; }

	.decisions-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 0.5rem; }
	.decision-card { background: rgba(26, 20, 12, 0.5); border: 1px solid #3d2810; border-radius: 6px; padding: 0.9rem 1rem; }
	.d-title { font-size: 1rem; font-family: 'Lexend'; letter-spacing: -0.5px; text-transform: uppercase; color: #c9973a; margin: 0 0 0.5rem; line-height: 1.35; }
	.d-body { font-size: 1rem; color: #7a5a38; line-height: 1.6; font-family: 'Inter'; letter-spacing: -0.3px; margin: 0; }

	/* ── Shared ── */
	.divider { display: flex; align-items: center; margin: 2rem 0; }
	.divider::before, .divider::after { content: ''; flex: 1; height: 1px; background: rgba(92, 61, 30, 0.4); }
	.diamond { display: inline-block; width: 7px; height: 7px; background: #7a5a20; transform: rotate(45deg); margin: 0 12px; flex-shrink: 0; }
	.signature { margin-top: 2.5rem; padding-top: 1rem; border-top: 1px solid rgba(92, 61, 30, 0.35); text-align: right; }
	.sig-name { font-size: 0.6rem; letter-spacing: 3px; color: #7a5a20; text-transform: uppercase; margin: 0 0 0.25rem; }
	.sig-date { font-size: 0.75rem; color: #5a3a18; font-style: italic; }
