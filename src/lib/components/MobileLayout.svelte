<script lang="ts">
	...
	$: isPanelEvent = $eventScreen.type === 'npc' || $eventScreen.type === 'location_event';
...
<div class="mobile-root" on:pointerdown={() => { if (dpadActive) { clearTimeout(dpadTimer); dpadTimer = setTimeout(() => dpadActive = false, 2000); }}}>
	...
	<div class="map-area">
		...
		{#if activeQuestCount > 0}
			<button class="quest-strip" on:click={() => { questDrawerOpen = !questDrawerOpen; }}>
		...
		<div
			class="dpad-float"
			class:active={dpadActive}
			on:pointerdown|stopPropagation={onDpadTouch}
		>
			<div class="dpad-grid">
		...
		<button
			class="log-fab"
			on:click={() => { logDrawerOpen = !logDrawerOpen; questDrawerOpen = false; }}
			title="Messages"
		>
	...
	<nav class="bottom-nav">
		<button class="nav-btn" class:active={activeTab === 'map'}       on:click={() => navigate('/map', 'map')}><img src="/game_icons/map.png" alt="Map" /><span>Map</span></button>
		<button class="nav-btn" class:active={activeTab === 'inventory'} on:click={() => navigate('/inventory', 'inventory')}><img src="/game_icons/inventory.png" alt="Inventory" /><span>Bag</span></button>
		<button class="nav-btn" class:active={activeTab === 'journal'}   on:click={() => navigate('/journal', 'journal')}><img src="/game_icons/journal.png" alt="Journal" /><span>Journal</span></button>
		<button class="nav-btn" class:active={activeTab === 'shop'}      on:click={() => navigate('/shop', 'shop')}><img src="/game_icons/shop.png" alt="Shop" /><span>Shop</span></button>
		<button class="nav-btn" class:active={activeTab === 'settings'}  on:click={() => navigate('/settings', 'settings')}><img src="/game_icons/settings.png" alt="Settings" /><span>Menu</span></button>
	</nav>
</div>
...
<style>
	...
	.loading {
		...
		height: 100%; color: #aaa; font-family: monospace;
	}
	...
	.hp-bars { display: flex; flex-direction: column; gap: 3px; max-width: 160px; }

	/* Quest strip */
	.quest-strip {
		position: absolute; top: 0; right: 0; z-index: 21;
		display: flex; align-items: center; gap: 6px;
		background-color: rgba(0,0,0,0.65);
		color: #facc15; border: none; padding: 5px 10px;
		font-family: var(--font-family-pixel); font-size: 0.65rem;
		cursor: pointer; border-bottom-left-radius: 8px;
	}
	.quest-strip-icon { width: 14px; height: 14px; }
	...
	.dpad-float {
		...
		opacity: 0.20;
		filter: saturate(0) blur(5px) brightness(.7);
	...
	.map-area :global(.bottom-right) { display: none; }
	...
	.log-fab img { width: 32px; height: 32px; }
	...
	.drawer-handle {
		...
		font-family: var(--font-family-pixel); font-size: 0.75rem;
	...
	.drawer-backdrop {
		...
		z-index: 49; background: transparent;
	}

	.quest-drawer { max-height: 45%; }
	.log-drawer {
		max-height: 60%;
	...
	/* Bottom nav */
	.bottom-nav {
		flex-shrink: 0; display: flex; height: 58px;
	...
	.nav-btn {
		...
		color: #888; font-family: var(--font-family-pixel);
		font-size: 0.55rem; transition: color 0.15s;
		-webkit-tap-highlight-color: transparent; padding: 4px 0;
	}
	.nav-btn img { width: 22px; height: 22px; opacity: 0.6; transition: opacity 0.15s; }
	.nav-btn.active { color: #fff; }
	.nav-btn.active img { opacity: 1; }
	.nav-btn:active { background-color: rgba(255,255,255,0.05); }
</style>