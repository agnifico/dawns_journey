<script lang="ts">
	...
	import { equipItem, getItemById, useItem, countInventoryItem } from '$lib/services/InventoryService';
	...
	const ELEMENTAL_MATERIAL_IDS = new Set([
		'citrine','turquoise','emerald','sapphire','amethyst','ruby',
		'red_bar','purple_bar','light_bar','earth_bar','sky_bar','water_bar',
	]);
	const SIGIL_IDS = new Set([
		'water_sigil','wind_sigil','earth_sigil','light_sigil','dark_sigil','fire_sigil',
	]);

	function isMaterial(item: Item) {
		return ELEMENTAL_MATERIAL_IDS.has(item.id) || item.flags?.includes('bar') || item.flags?.includes('gem');
	}
	function isSigil(item: Item) {
		return SIGIL_IDS.has(item.id) || item.flags?.includes('sigil') || item.flags?.includes('boss_material');
	}
	...
	const groupedInventory = derived(
		[filteredInventory, inventoryTab],
		([$filtered, $tab]) => {
	...
	function selectItem(item: Item) { $activeItem = item; actionSheetItem = null; }
	function closeDrawer() { $activeItem = null; }
...
<div class="inventory">
	...
	{#if $inventoryTab === 'relics'} <InventoryFilterBar isWeaponTab={false} />{/if}
			...
			<div class="item-grid">
				{#if $inventoryTab === 'general'}
								...
								>
									...
									{#if actionSheetItem?.id === item.id}
										<div class="item-action-sheet">
											{#if isConsumable(item)}<button class="ias-btn ias-use" on:click|stopPropagation={() => { handleUse(item); closeActionSheet(); }}>Use</button>{/if}
											{#if isEquippable(item)}<button class="ias-btn ias-equip" on:click|stopPropagation={() => { equipItem(item.instanceId); closeActionSheet(); }}>Equip</button>{/if}
										</div>
									{/if}
								</div>
							{/each}
					...
					{#if $groupedInventory.sigils.length > 0}
						<button
							class="section-header"
							on:click={() => sigilsExpanded = !sigilsExpanded}
						>
		...
		{#if drawerItem}
			<div class="item-drawer">
				...
				{#if drawerItem.gearPassives && drawerItem.gearPassives.length > 0}
					...
					{#each drawerItem.gearPassives as passive}
						<GearPassive
...
<style>
	...
	.tabs button.active { background: var(--color-accent); color: var(--text-white); }
	.sub-tabs {
		display: flex;
		flex-shrink: 0;
	...
	.section-header:hover { background: rgba(200,169,110,0.08); color: rgba(200,169,110,0.85); }
	.sh-label { font-size: 0.6rem; letter-spacing: 0.08em; text-transform: uppercase; flex: 1; }
	.sh-count { font-size: 0.6rem; color: rgba(200,169,110,0.4); }
	.sh-chevron { font-size: 0.8rem; transition: transform 0.15s; flex-shrink: 0; }
	...
	.grid-item:hover { border-color: rgba(200,169,110,0.35); background: rgba(60,50,30,0.9); }
	.grid-item.selected { border-color: rgba(200,169,110,0.8); box-shadow: 0 0 0 1px rgba(200,169,110,0.4); }

	/* ── Action sheet ── */
	.sheet-backdrop { position: fixed; inset: 0; z-index: 98; }
	.item-action-sheet {
		position: absolute;
		bottom: calc(100% + 4px);
		left: 50%;
	...
	.ias-btn { padding: 4px 10px; border-radius: 4px; font-family: var(--font-family-pixel); font-size: 0.55rem; cursor: pointer; border: 1px solid; transition: background 0.1s; }
	...
	.drawer-close { position: absolute; top: 6px; right: 8px; background: none; border: none; color: rgba(200,169,110,0.4); font-size: 0.7rem; cursor: pointer; padding: 2px 4px; }
	.drawer-close:hover { color: rgba(200,169,110,0.8); }
	.drawer-img { display: flex; border-radius: 6px; }
	.drawer-header { display: flex; align-items: center; justify-content: space-between; gap: 6px; }
	.drawer-name { margin: 0; font-family: var(--font-family-pixel); font-size: 1rem; font-weight: 400; color: #c8a96e; flex-grow: 1; }
	.drawer-desc { font-family: "Lexend"; font-style: italic; font-size: 0.85rem; color: var(--text-muted); line-height: 1.2; margin: 0; }
	.drawer-section-label { font-family: var(--font-family-pixel); font-size: 0.55rem; font-weight: 600; letter-spacing: 0.1em; text-transform: uppercase; color: rgba(200,169,110,0.4); margin-top: 2px; }
	.drawer-stats { display: flex; flex-direction: column; gap: 4px; }
	.drawer-set { display: flex; flex-direction: column; gap: 3px; }
	.set-bonus-line { font-family: var(--font-family-pixel); font-size: 0.6rem; color: var(--color-accent); padding: 4px 8px; background: rgba(0,0,0,0.25); border-radius: 4px; }
	.set-pieces { color: var(--text-muted); margin-right: 4px; }
	.drawer-effects { display: flex; flex-wrap: wrap; gap: 4px; }
	.drawer-actions { display: flex; gap: 6px; margin-top: auto; padding-top: 4px; border-top: 1px solid rgba(200,169,110,0.1); }
	.drawer-btn { flex: 1; padding: 6px 0; border-radius: 5px; font-family: var(--font-family-pixel); font-size: 0.6rem; cursor: pointer; border: 1px solid; text-align: center; transition: background 0.1s; }
