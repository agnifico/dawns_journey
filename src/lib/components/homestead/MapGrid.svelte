<script lang="ts">
	import {
		...
		type HomesteadPlots,
		type PlotData
	} from '$lib/services/HomesteadDataService';
	...
	const currentPlots = derived(currentEnvironment, ($currentEnvironment) => {
...
<div class="map-grid-overlay" class:pan-down={showBottomHalf}>
	{#each $currentPlots as plot (plot.id)}
		<div
			...
			style="left: {plot.x * 1.25}px; top: {plot.y * 1.25}px; width: {plot.width *
				1.25}px; height: {plot.height * 1.25}px;"
